...
var DEFAULT_REPORT_URL = '/csp-report-uri';
...
csp.reporter = function reporter (uri, handler) {

    if (arguments.length === 1) {
        handler = uri;
        uri = DEFAULT_REPORT_URL;
    }

    return function reporter (req, res, next) {

        var method = req.method.toLowerCase();
        var path = url.parse(req.url).pathname;
        if ((method != 'post') && (path != uri)) {
            next();
        }

        req.cspReport = JSON.parse(req.body);
